...
void display_stat(ui_t *ui, player_t *player)
{
    ...
    display_with_ascii("WPM", ui->result->wpm_per_word[last_word_index], column_offset - 4, ui->row / 3 + 1);
    display_with_ascii("ACC", ui->result->data[ACCURACY], column_offset - 4, ui->row / 3 + 7);
